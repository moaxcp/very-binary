...
public final class Int32ArrayType extends PrimitiveArrayType<Int32ArrayType, Integer> {

  private final int @Nullable [] constantValue;

  public Int32ArrayType(int position, int @Nullable [] constantValue, @Nullable Expression lengthExpression, @Nullable Expression byteLengthExpression) {
    super(position, INT32, lengthExpression, byteLengthExpression);
    this.constantValue = constantValue;
    checkConstant();
  }

  @Override
  public Int32ArrayType copy(int position) {
    return new Int32ArrayType(position, constantValue, getLengthExpression(), getByteLengthExpression());
  }
  ...
  public int[] getInt32(Pointer<?, ? extends Type<?>> pointer, long index, long length) {
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, int value) {
    ...
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getInt32(getOffset(pointer, index), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, int[] values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getInt32(getOffset(pointer), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, List<Integer> values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getInt32List(getOffset(pointer), getLength(pointer));
  ...
  public void allocate(Pointer<?, ? extends Type<?>> pointer) {
    if (this.isConstant()) {
      pointer.getByteArray().addInt32(getOffset(pointer), constantValue);
    } else {
