...
public final class Int64ArrayType extends PrimitiveArrayType<Int64ArrayType, Long> {

  private final long @Nullable [] constantValue;

  public Int64ArrayType(int position, long @Nullable [] constantValue, @Nullable Expression lengthExpression, @Nullable Expression byteLengthExpression) {
    super(position, INT64, lengthExpression, byteLengthExpression);
    this.constantValue = constantValue;
    checkConstant();
  }

  @Override
  public Int64ArrayType copy(int position) {
    return new Int64ArrayType(position, constantValue, getLengthExpression(), getByteLengthExpression());
  }
  ...
  public long[] getInt64(Pointer<?, ? extends Type<?>> pointer, long index, long length) {
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, long value) {
    ...
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getInt64(getOffset(pointer, index), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, long[] values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getInt64(getOffset(pointer), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, List<Long> values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getInt64List(getOffset(pointer), getLength(pointer));
  ...
  public void allocate(Pointer<?, ? extends Type<?>> pointer) {
    if (this.isConstant()) {
      pointer.getByteArray().addInt64(getOffset(pointer), constantValue);
    } else {
