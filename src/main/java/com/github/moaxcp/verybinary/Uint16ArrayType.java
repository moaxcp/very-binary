...
public final class Uint16ArrayType extends PrimitiveArrayType<Uint16ArrayType, Integer> {

  private final int @Nullable [] constantValue;

  public Uint16ArrayType(int position, int @Nullable [] constantValue, @Nullable Expression lengthExpression, @Nullable Expression byteLengthExpression) {
    super(position, UINT16, lengthExpression, byteLengthExpression);
    this.constantValue = constantValue;
    checkConstant();
  }

  @Override
  public Uint16ArrayType copy(int position) {
    return new Uint16ArrayType(position, constantValue, getLengthExpression(), getByteLengthExpression());
  }
  ...
  public int[] getUint16(Pointer<?, ? extends Type<?>> pointer, long index, long length) {
    checkArrayRange(pointer, index, index + length);
    return pointer.getByteArray().getUint16(getOffset(pointer, index), length);
  }

  public List<Integer> getUint16List(Pointer<?, ? extends Type<?>> pointer) {
    var length = getLength(pointer);
    checkIndex(pointer, length - 1);
    return pointer.getByteArray().getUint16List(getOffset(pointer), length);
  }

  public List<Integer> getUint16List(Pointer<?, ? extends Type<?>> pointer, long index, long length) {
    checkArrayRange(pointer, index, index + length);
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, int value) {
    ...
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getUint16(getOffset(pointer, index), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, int[] values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getUint16(getOffset(pointer), getLength(pointer));
      var newValue = new int[old.length + values.length];
      if (index == 0) {
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, List<Integer> values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getUint16List(getOffset(pointer), getLength(pointer));
      var newValue = new ArrayList<Integer>(values.size());
  ...
  public void checkForConstantValues(Pointer<?, ? extends Type<?>> pointer, long index, List<Integer> values) {
    if (this.isConstant()) {
  ...
  public void allocate(Pointer<?, ? extends Type<?>> pointer) {
    if (this.isConstant()) {
      pointer.getByteArray().addUint16(getOffset(pointer), constantValue);
    } else {
