...
public final class Uint8ArrayType extends PrimitiveArrayType<Uint8ArrayType, Short> {

  private final short @Nullable [] constantValue;

  public Uint8ArrayType(int position, short @Nullable [] constantValue, @Nullable Expression lengthExpression, @Nullable Expression byteLengthExpression) {
    super(position, UINT8, lengthExpression, byteLengthExpression);
    this.constantValue = constantValue;
  }

  @Override
  public Uint8ArrayType copy(int position) {
    return new Uint8ArrayType(position, constantValue, getLengthExpression(), getByteLengthExpression());
  }
  ...
  public short[] getUint8(Pointer<?, ? extends Type<?>> pointer, long index, long length) {
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, short value) {
    ...
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getUint8(getOffset(pointer, index), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, short[] values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getUint8(getOffset(pointer), getLength(pointer));
  ...
  private void setUnchecked(ValueChangeReason reason, Pointer<?, ? extends Type<?>> pointer, long index, List<Short> values) {
    if (!getValueChangeListeners().isEmpty()) {
      var old = pointer.getByteArray().getUint8List(getOffset(pointer), getLength(pointer));
  ...
  public void allocate(Pointer<?, ? extends Type<?>> pointer) {
    if (isConstantValue(pointer.getType())) {
      pointer.getByteArray().addUint8(getOffset(pointer), constantValue);
    } else {
