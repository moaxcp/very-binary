...
import static com.github.moaxcp.verybinary.math.BooleanGatherer.greaterThanOrEqual;

public final class GreaterThanOrEqualExpression implements ArithmeticExpression, MultiExpression<ArithmeticExpression, ArithmeticValue> {

  private final List<ArithmeticExpression> expressions;

  public GreaterThanOrEqualExpression(List<ArithmeticExpression> expressions) {
  ...
  public ArithmeticValue constantValue(ComplexType<?> parent) {
    return expressions.stream().map(e -> e.constantValue(parent)).gather(greaterThanOrEqual()).findAny().orElse(BoolValue.FALSE);
  }

  @Override
  public ArithmeticValue defaultValue(ComplexType<?> parent) {
    return expressions.stream().map(e -> e.defaultValue(parent)).gather(greaterThanOrEqual()).findAny().orElse(BoolValue.FALSE);
  }

  @Override
  public ArithmeticValue evaluate(Pointer<?, ? extends Type<?>> pointer) {
    return expressions.stream().map(e -> e.evaluate(pointer)).gather(greaterThanOrEqual()).findAny().orElse(BoolValue.FALSE);
  }
