...
public final class LessThanOrEqualExpression implements ArithmeticExpression, MultiExpression<ArithmeticExpression, ArithmeticValue> {

  private final List<ArithmeticExpression> expressions;

  public LessThanOrEqualExpression(List<ArithmeticExpression> expressions) {
  ...
  public ArithmeticValue constantValue(ComplexType<?> parent) {
    return expressions.stream().map(e -> e.constantValue(parent)).gather(lessThanOrEqual()).findAny().orElse(BoolValue.FALSE);
  }

  @Override
  public ArithmeticValue defaultValue(ComplexType<?> parent) {
    return expressions.stream().map(e -> e.defaultValue(parent)).gather(lessThanOrEqual()).findAny().orElse(BoolValue.FALSE);
  }
