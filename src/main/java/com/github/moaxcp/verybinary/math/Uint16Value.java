...
public final class Uint16Value extends ArithmeticValue {
  final int value;

  private Uint16Value(int value) {
    this.value = value;
  }

  public static Uint16Value uint16Value(int value) {
  ...
  public ArithmeticValue sum(ArithmeticValue other) {
    return switch (other) {
      case BoolValue v -> uint32Value(value + v.toInt());
  ...
  public ArithmeticValue multiply(ArithmeticValue other) {
    return switch (other) {
      case BoolValue v -> uint32Value((long) value * v.toInt());
      case Float32Value v -> float32Value(value * v.value);
      case Float64Value v -> float64Value(value * v.value);
      case Int8Value v -> int32Value(value * v.value);
      case Int16Value v -> int32Value(value * v.value);
  ...
  public ArithmeticValue valueOfBit(long bit) {
    if (bit < 0 || bit > 17) {
      throw new IllegalArgumentException("Bit index must be between 0 and 17");
    }
  ...
  public boolean equals(Object o) {
    ...
    Uint16Value other = (Uint16Value) o;
