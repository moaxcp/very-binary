...
public final class ValueOfBit implements ArithmeticExpression {

  private final int position;
  private final long bit;

  public ValueOfBit(int position, long bit) {
  ...
  public ArithmeticValue constantValue(ComplexType<?> parent) {
    var value =  switch (parent.getType(position)) {
  ...
  public ArithmeticValue defaultValue(ComplexType<?> parent) {
    var value = int64Value(((LengthType<?, ?>) parent.getType(position)).defaultLengthValue());
